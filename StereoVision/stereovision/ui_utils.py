...
class BMTuner(object):
    ...
    def _initialize_trackbars(self):
        ...
        for parameter in self.block_matcher.parameter_maxima.keys():
            ...
            if not maximum:
                maximum = self.shortest_dimension # WHAT THE HECK!!
    ...
    def update_disparity_map(self):
        ...
        disparity2 = cv2.normalize(disparity, disparity,alpha=0, beta= 255, norm_type = cv2.NORM_MINMAX, dtype = cv2.CV_8U)
        cv2.imshow(self.window_name, disparity2)
        cv2.waitKey()
